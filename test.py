...
def crawl_folder(folder, dataset="nyu"):
    imgs = sorted(
        (folder / "color/").files("*.png") + (folder / "color/").files("*.jpg")
    )
...
class TestSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        depth = (
            torch.from_numpy(imread(self.depths[index]).astype(np.float32)).float()
            / 1000
        )
...
def compute_errors(gt, pred, dataset):
    ...
    for current_gt, current_pred in zip(gt, pred):
        ...
        rmse_log += torch.sqrt(
            torch.mean((torch.log(valid_gt) - torch.log(valid_pred)) ** 2)
        )
        log10 += torch.mean(
            torch.abs((torch.log10(valid_gt) - torch.log10(valid_pred)))
        )
...
def main():
    ...
    test_dataset = TestSet(
        hparams.input_dir, transform=inference_transform, dataset=hparams.dataset_name
    )
    print("{} samples found in test scenes".format(len(test_dataset)))

    test_loader = DataLoader(
        test_dataset, batch_size=1, shuffle=False, num_workers=8, pin_memory=True
    )
